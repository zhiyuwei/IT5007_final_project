...
async function bookAdd(_, { book }) {
  ...
  if(data){
    ...
    const result = await db.collection('books').insertOne({id:book.id,total:book.total,author:book.author,
      publisher:book.publisher,title:book.title,avail:number,image:book.image,intro:book.intro,
      date:book.date,location:book.location,place:book.place});
...
async function bookReturn(_, { loan }) {
  ...
  if(!data){
    throw new UserInputError("No reader borrows this book. Please enter again.")
  }
...
async function loanAdd(_,{loan}) {
  const data= await db.collection('loans').findOne({title:loan.title, reader:loan.reader})
  if (data){
    throw new UserInputError("You have already borrow this book. Please borrow other books")
  }
  else{
    ...
    const savedloan = await db.collection('loans').findOne({ _id: result.insertedId });
    return savedloan;
  }
...
async function displayLoan(_,{username}){
  const loans = await db.collection('loans').find({reader:username}).toArray();
  return loans;
}
